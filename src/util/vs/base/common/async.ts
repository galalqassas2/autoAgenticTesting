...
export class AsyncIterableSource<T> {
	...
	push(value: T): void {
		if (this._finished) {
			throw new Error('AsyncIterableSource already finished');
		}
		if (this._listeners.length > 0) {
			const listener = this._listeners.shift()!;
			listener.resolve({ value, done: false });
		} else {
			this._results.push(value);
		}
	}

	// Alias for push - some code uses emitOne
	...
	resolve(): void {
		...
		while (this._listeners.length > 0) {
			const listener = this._listeners.shift()!;
			listener.resolve({ value: undefined as any, done: true });
		}
	}
		...
		return {
			next: (): Promise<IteratorResult<T>> => {
				...
				if (this._results.length > 0) {
					return Promise.resolve({ value: this._results.shift()!, done: false });
				}
				if (this._finished) {
